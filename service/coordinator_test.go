...
func Test_mergeResults(t *testing.T) {
	...
	stmt := mustGetSelect(`SELECT mean("value"), top("value", 1), sum("value"), abs(round(mean("value") * -3.55)) FROM sales where time < now() group by time(1d) `)
	...
	assert.Equal(t, []float64{14}, tree.Fields[3].Root.Next(source))
	source.Step()
	...
	assert.Equal(t, []float64{36}, tree.Fields[3].Root.Next(source))
	source.Reset()
